...
struct Player{
	...
	SDL_Rect position; //esse pedaço tem que sair daqui depois
...
static void player_load_textures(SDL_Renderer *ren, Player *player);
static void player_destroy_textures(Player *player);

static RankInfo escoteiro_info, aventureiro_info, pesquisador_info;
static int init_flag = 0;

Player *create_player(SDL_Renderer *ren) {
    ...
    *player = (Player){
	...
	player_load_textures(ren, player);
    return player;
}
...
static void player_load_textures(SDL_Renderer *ren, Player *player){
	player->texture = IMG_LoadTexture(ren, "assets/img/player/sprite_player.png");
    assert(player->texture != NULL);
}

void player_set_idle(Player *player){
	player->mov = IDLE;
}
...
void update_collected_treasures(Player *player){
	...
	if (player->stats.collected_treasures_counter == MAX_TREASURES){
...
void destroy_player(Player *player){
	player_destroy_textures(player);
	free(player);
}
...
int get_player_tam_invent (Player *player){
	RankInfo info = get_rank_info (player->rank);
	return info.max_items_inventory;
}
...
int get_collected_artifacts(Player *player){
	return player->stats.collected_artifacts_counter;
}

int get_collected_potions(Player *player){
	return player->stats.collected_potions_counter;
}

int get_consumed_artifacts(Player *player){
	return player->stats.consumed_artifacts_counter;
}
...
void player_render(Player *player, SDL_Renderer *ren, int cam_x, int cam_y){
    ...
    if (player->mov == IDLE) {
        col = 0;
        player->anima_timer = 0;
    } else {
        player->anima_timer++;
        
        int walk_cycle[] = { 0, 1, 0, 2 }; 
        int index = (player->anima_timer / ANIMA_SPEED) % 4;
        col = walk_cycle[index];
    }
    ...
    SDL_RenderCopy(ren, player->texture, &src_rect, &dst_rect);
}
